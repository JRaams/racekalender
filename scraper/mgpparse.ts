...
function findCircuit(circuitName: string, eventName: string): Circuit | null {
  ...
  const exactMatch = mgpCircuits.find((circuit: Circuit) => circuit.name.toLowerCase() === circuitNameLower);
  if (exactMatch) return exactMatch;

  // Try partial match
  const partialMatch = mgpCircuits.find(
    (circuit: Circuit) =>
      circuit.name.toLowerCase().includes(circuitNameLower) || circuitNameLower.includes(circuit.name.toLowerCase()),
  );
  if (partialMatch) return partialMatch;

  // Try matching by event name/city
  if (eventNameLower) {
    const cityMatch = mgpCircuits.find(
      (circuit: Circuit) =>
...
async function parseRaceWeek(meta: MotoGPWeekendMeta): Promise<RaceWeek | null> {
  ...
  const motogpCategoryData = scheduleData.event_categories?.find((cat: any) => cat.category_id === motogpCategoryId);
